...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, config=None, filename=None, output=None, output_file=None, output_dir=None):
        ...
        self.landmarks_indexes = [17, 22, 27, 36, 42, 48, 68]
        self.landmarks_categories = ["jaw_line", "left_eyebrow", "right_eyebrow", "nose", "left_eye", "right_eye", "mouth"]
        self.landmarks = None
    ...
    def load_json(self, img_id):
        ...
        if os.path.exists("../annotations/" + self.art_id + "_" + self.img_id + "_2D.json"):
            ...
            file = "../data/" + self.shard_id + "/" + self.art_id + "/" + self.img_id + ".json"
        with open(file) as f:
            data = json.load(f)
            self.landmarks = np.asarray(data['landmarks'])
            self.visibility = np.asarray(data['visibility'])
            start_index = 0
            for category, end_index in zip(self.landmarks_categories, self.landmarks_indexes):
                tmp = []
                for ldk, visi in zip(self.landmarks[start_index:end_index], self.visibility[start_index:end_index]):
                    if visi:
                        tmp.append(ldk)
                if len(tmp) > 0:
                    shapes.append({'label': category, 'points': tmp, 'shape_type': 'linestrip', 'flags': {},
    ...
    def loadFile(self, filename=None):
        ...
        shapes = self.load_json(self.img_id)
    ...
    def changeOutputDirDialog(self, _value=False):
        ...
        output_dir = QtWidgets.QFileDialog.getExistingDirectory(self,
                                                                self.tr(
                                                                    "%s - Save/Load Annotations in Directory") % __appname__,
                                                                default_output_dir,
    ...
    def openDirDialog(self, _value=False, dirpath=os.path.join("..", "data")):
        if not self.mayContinue():
            return
        ...
        defaultOpenDirPath = os.path.join("..", "data")
        targetDirPath = str(
